...
1. **根据结局类型展开不同的故事线**：
   - TRUE_END（公布真相）：玩家当众揭露真相后，李德福和赵虎会如何反应？其他NPC（根据信任度）会如何应对？最终结果取决于玩家的人际关系。
     * 如果有NPC信任度>=70，该NPC可能在关键时刻出手相助（比如顾琼亮出武器、韩子敬挺身作证等）
...
2. **必须包含至少2个NPC的具体反应**（根据他们对玩家的信任度来决定态度）
...
async def handle_accuse(user_input, request, current_state, model_id):
    ...
    # --- 案件卷宗报告 ---
        ...
        report_data = {
            ...
            "truth_reveal": {
                "victim": "张三（真名：李福运）—— 前掌印太监，马嵬坡之变知情人",
                "killer": "赵虎 —— 奉李德福之命，用金镶玉拂尘中的乌金丝勒杀",
                "mastermind": "李德福 —— 奉皇帝密旨清除知情人",
                "motive": "张三知道杨贵妃在马嵬坡未死的秘密，皇帝下令灭口",
                "others": [
                    "顾琼：杨氏后人，本想毒杀李德福复仇，意外撞见尸体后逃走",
                    "韩子敬：写了反诗的书生，去后院埋诗稿时看到尸体，吓得跑回房烧稿",
                    "清虚子：江湖骗子，收了张三的钱算命，发现尸体时吓得掉了拂尘"
                ]
            }
        }
